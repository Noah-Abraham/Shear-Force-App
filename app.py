...
if bolts:
    ...
    # Calculate per-bolt loads
    tensile_loads = [abs(vz) for _, _, _, _, vz in shear_forces]
    shear_loads = [np.hypot(vx, vy) for _, _, vx, vy, _ in shear_forces]
    ...
    force_df["Shear Load (kN)"] = shear_loads
    force_df["Tensile Load (kN)"] = tensile_loads
