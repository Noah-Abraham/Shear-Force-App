...
if bolts:
    ...
    for i, (x, y, vx, vy, vz) in enumerate(shear_forces):
        if view_option == "XY View":
            ...
            ax.plot(x, y, 'ro')
            ax.text(x - 0.15 + (-1)**i * 0.1, y - 0.15 + (-1)**i * 0.1, f"Bolt {i+1} ({vx:.3f} kN, {vy:.3f} kN)", fontsize=7, color='black', ha='right', va='top')
            
        elif view_option == "XZ View":
            ax.quiver(x, 0, vx * vector_display_scale, vz * vector_display_scale, angles='xy', scale_units='xy', scale=1, color='green')
            ax.plot(x, 0, 'ro')
            ax.text(x - 0.15 + (-1)**i * 0.1, vz * vector_display_scale - 0.15, f"Bolt {i+1} ({vx:.3f} kN, {vz:.3f} kN)", fontsize=7, color='black', ha='right', va='top')
            
        elif view_option == "YZ View":
            ax.quiver(y, 0, vy * vector_display_scale, vz * vector_display_scale, angles='xy', scale_units='xy', scale=1, color='purple')
            ax.plot(y, 0, 'ro')
            ax.text(y - 0.15 + (-1)**i * 0.1, vz * vector_display_scale - 0.15, f"Bolt {i+1} ({vy:.3f} kN, {vz:.3f} kN)", fontsize=7, color='black', ha='right', va='top')
